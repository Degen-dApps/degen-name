<template>
  <div class="mb-3 row domain-data mt-4" v-if="customData" :key="dataKey" v-for="(dataValue, dataKey) in customData">
    ...
    <div class="col-sm-9 punk-text text-break">
      ...
      <span v-else-if="dataKey=='farcaster'"><a target="_blank" :href="getWarpcastUrl(dataValue)">{{dataValue}}</a></span>
        ...
        <div class="modal-body">
          ...
          <div class="input-group mb-3 mt-4" v-if="fields" v-for="(item, index) in fields">
            <input 
              ...
              placeholder="Enter key (like farcaster)"
            >
...
export default {
  ...
  methods: {
    ...
    getWarpcastUrl(entry) {
      if (typeof entry === 'string') {
        if (entry.startsWith("http")) {
          return entry;
        } else if (entry.startsWith("www")) {
          return "https://" + entry;
        } else if (entry.startsWith("warpcast.com")) {
          return "https://" + entry;
        } else if (entry.startsWith("@")) {
          return "https://warpcast.com/" + entry.replace("@", "");
        } else {
          return "https://warpcast.com/" + entry;
        }
      }
    },
  ...
  watch: {
    domainData() {
      ...
      if(this.fields.findIndex(x => x.dataKey == "farcaster") === -1) {
        this.fields.push({dataKey: "farcaster", dataValue: "", valuePlaceholder: "Enter your Farcaster handle"});
      }
